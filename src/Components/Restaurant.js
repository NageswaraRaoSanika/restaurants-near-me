...
class RecipeReviewCard extends React.Component {
  ...
  render() {
    const { classes, restaurant, openModal,size } = this.props;
    const restaurantData = (restaurant.restaurant) ? restaurant.restaurant : restaurant;
    const imageURL = restaurantData.thumb ? restaurantData.thumb : Image;
    return restaurantData.id ? (<Grid item xs={12} sm={size}>
          <Card className={classes.card} onClick={ () => openModal(restaurantData) }>
            <CardHeader
              avatar={
